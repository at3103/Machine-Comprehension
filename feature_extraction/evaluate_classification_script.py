...
def evaluate_ml_result(file):
    ...
    with open('../data/predictions/lr/check/Full_data_classification.csv', 'rb') as csvfile:
        with open('../data/predictions/lr/check/Full_data_classification_Y.csv', 'rb') as csvfile_y:
            inputreader = csv.reader(csvfile, delimiter=',')
            inputreader.next()
            outputreader = csv.reader(csvfile_y, delimiter=',')
            outputreader.next()
            for row in inputreader:
                for row1 in outputreader:
                    predicted_F1_score = str(row1[-1])
                    ground_truths = row[-2]
                    qid = row[-3]
